...
## Task 18: surface new backend error codes

Backend gained two new error responses (`422 not_clinical_transcript` and `429 rate_limited`) when the classifier guardrail and rate limiting were added (backend Tasks 21 & 22). Frontend needs to surface them.

**Files:**
- Modify: `frontend/src/api/types.ts` — extend `ApiErrorCode` union with `not_clinical_transcript` and `rate_limited`; add optional `reason?: string` to `ApiError`.
- Modify: `frontend/src/components/states/ErrorAlert.tsx` — add the two new entries to the `MESSAGES` record. For `not_clinical_transcript`, render `error.body.reason` as a caption underneath the main message (mirrors the `transcript_too_long` pattern that renders `length` / `max_length`).
- Modify: `frontend/src/components/states/ErrorAlert.test.tsx` — add `renders the not-clinical message with the reason` and `renders the rate-limited message`.

- [ ] **Step 1: Update types**

In `frontend/src/api/types.ts`, replace the existing `ApiErrorCode` union to add the two new codes, and add a `reason?: string` field to `ApiError`:
...
- [ ] **Step 2: Add MESSAGES entries**

In `frontend/src/components/states/ErrorAlert.tsx`, add two entries in `MESSAGES`:
...
Place them after `transcript_too_long` and before `extraction_failed` (matches the order in `ApiErrorCode`).

- [ ] **Step 3: Render `reason` for not_clinical_transcript**

Below the main `Typography` line in the `<Alert>` branch, add:
...
(Mirrors the `transcript_too_long` pattern that renders `length` and `max_length`.)
...
Expected: all tests pass (34 previous + 2 new = 36).

```bash
cd /Users/ayano/vetrec/frontend && pnpm coverage
```

Expected: coverage stays >= 80%.

- [ ] **Step 6: Commit**

```bash
cd /Users/ayano/vetrec
git add frontend/src/api/types.ts frontend/src/components/states/ErrorAlert.tsx frontend/src/components/states/ErrorAlert.test.tsx
git commit -m "surface new error codes in frontend"
```

## Self-review for Task 18

- `ApiErrorCode` union has 8 codes (added `not_clinical_transcript`, `rate_limited`)?
- `reason?: string` on `ApiError`?
- Two new `MESSAGES` entries in correct order?
- `not_clinical_transcript` renders `reason` as caption?
- Two new ErrorAlert tests pass?
- 36 total tests pass, coverage >= 80%?
