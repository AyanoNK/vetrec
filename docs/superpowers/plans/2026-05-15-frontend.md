...
**Testing strategy:**
- Vitest watch on `npm test`, single run on `npm test -- --run`, coverage on `npm run coverage` (80% gate via threshold in vitest config).
...
```bash
cd /Users/ayano/vetrec/frontend && npm install
```

- [ ] **Step 11: Verify build**

```bash
cd /Users/ayano/vetrec/frontend && npm run build
```
...
```bash
cd /Users/ayano/vetrec/frontend && timeout 5 npm run dev || true
```
...
If any of these versions are unavailable (e.g., npm only has a newer major), use the latest compatible version. The plan's versions are floors, not pins.
...
```bash
cd /Users/ayano/vetrec/frontend && npm install --save-dev \
  vitest@^2.1.0 \
...
```bash
cd /Users/ayano/vetrec/frontend && npm run test:run
```
...
```bash
cd /Users/ayano/vetrec/frontend && npm run test:run -- src/api/client.test.ts
```
...
```bash
cd /Users/ayano/vetrec/frontend && npm run test:run -- src/hooks/useExtractTimeline.test.tsx
```
...
```bash
cd /Users/ayano/vetrec/frontend && npm run test:run -- src/lib/eventStyling.test.ts
```
...
```bash
cd /Users/ayano/vetrec/frontend && npm run test:run -- src/components/EventCard.test.tsx
```
...
```bash
cd /Users/ayano/vetrec/frontend && npm run test:run -- src/components/TimelineView.test.tsx
```
...
```bash
cd /Users/ayano/vetrec/frontend && npm run test:run -- src/components/TranscriptForm.test.tsx
```
...
```bash
cd /Users/ayano/vetrec/frontend && npm run test:run -- src/components/states
```
...
```bash
cd /Users/ayano/vetrec/frontend && npm run test:run -- src/App.test.tsx
```
...
```bash
cd /Users/ayano/vetrec/frontend && npm run coverage
```
...
```bash
cd /Users/ayano/vetrec/frontend && (npm run dev &) && sleep 4
...
npm run coverage        # 80% gate; HTML report at frontend/coverage/index.html
\`\`\`
```
...
- [ ] `cd frontend && npm run coverage` — total coverage >= 80%.
